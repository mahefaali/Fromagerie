...
export default function DescendingTraceability() {
  ...
  return <>
    ...
    {!data && !loading && !error && <div className="mt-5 rounded-2xl border border-dashed border-[#d8d0bd] bg-[#f7f3e9] px-5 py-7 text-center"><Package className="mx-auto mb-3 size-8 text-[#756a57]"/><h2 className="text-lg font-semibold">Recherchez un lot pour retrouver ses destinations</h2></div>}
    {data && <div className="mt-5 space-y-4">
      <section className="rounded-2xl border border-[#b8bea2] bg-[#eeecdf] p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-[#756a57]">Lot de fabrication</p><h2 className="mt-1 font-mono text-xl font-bold">{data.numeroLot}</h2>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {[['Produit',data.quantiteProduite],['Livré',data.quantiteLivree],['Non vendu',data.quantiteNonVendue],['Disponible',data.quantiteDisponible],['Perdu',data.quantitePerdue]].map(([label,value])=><div key={label} className="rounded-xl border border-[#d8d0bd] bg-[#fffdf8] p-3"><p className="text-xs text-[#756a57]">{label}</p><p className="mt-1 text-xl font-bold">{value}</p></div>)}
      </section>
