...
export function LotCostCards({ lots, onSelect }: { lots: LotProductionCost[]; onSelect: (lot: LotProductionCost) => void }) {
  return <Card className={cardClass}><CardHeader><CardTitle>Coût par lot</CardTitle></CardHeader><CardContent>{lots.length === 0 ? <div className="h-32"><Empty /></div> : <div className="grid gap-4 md:grid-cols-2">{lots.map((lot) => <button key={lot.id} type="button" onClick={() => onSelect(lot)} className="rounded-xl border border-[#ddd4c6] bg-[#fffdf8] p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#c94b29] hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c94b29]"><div className="flex items-center justify-between"><strong>{lot.numeroLot}</strong><ChevronRight className="size-4 text-[#776954]" /></div><p className="mt-2 text-[#756650]">{lot.fromageNom}</p><div className="mt-5 flex items-end justify-between gap-4"><strong className="text-2xl">{money.format(lot.coutTotal)}</strong><span className="text-xs text-[#756650]">{number.format(lot.poidsTotal)} kg · {preciseMoney.format(lot.coutParKg)}/kg</span></div></button>)}</div>}</CardContent></Card>;
}

export function LotCostDialog({ lot, onClose }: { lot: LotProductionCost | null; onClose: () => void }) {
  return <Dialog open={lot !== null} onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="rounded-2xl"><DialogHeader><DialogTitle>Coût du lot {lot?.numeroLot}</DialogTitle></DialogHeader>{lot && <div className="grid grid-cols-2 gap-3 text-sm">{COST_DEFINITIONS.map((definition) => <div key={definition.label} className="rounded-xl bg-[#f7f3ec] p-3"><p className="text-[#8a7565]">{definition.label}</p><p className="mt-1 font-semibold">{preciseMoney.format(Number(lot[definition.key]))}</p></div>)}<div className="col-span-2 rounded-xl bg-[#28551f] p-4 text-white"><p>Coût total</p><p className="mt-1 text-xl font-semibold">{preciseMoney.format(lot.coutTotal)}</p></div></div>}</DialogContent></Dialog>;
}
