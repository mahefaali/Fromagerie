...
export function CaveEnvironmentFields({ values, update }: FieldsProps) {
  return <>
    <div className="grid grid-cols-2 gap-3">
      <NumberField id="cave-temp" label="Température (°C)" value={values.temperature} step="0.01" onChange={(value) => update("temperature", value)} />
      <NumberField id="cave-hum" label="Humidité (%)" value={values.humidite} step="0.01" min={0} max={100} onChange={(value) => update("humidite", value)} />
    </div>
    <div className="grid grid-cols-2 gap-3">
      <NumberField id="cave-age-min" label="Âge minimum (jours)" value={values.ageMinJours} min={0} onChange={(value) => update("ageMinJours", value)} />
      <NumberField id="cave-age-max" label="Âge maximum (jours)" value={values.ageMaxJours} min={1} onChange={(value) => update("ageMaxJours", value)} />
    </div>
...
function NumberField({ id, label, value, onChange, ...limits }: { id: string; label: string; value: number; onChange: (value: number) => void; min?: number; max?: number; step?: string }) {
  return <Field label={label} id={id}><Input id={id} type="number" value={value} onChange={(event) => onChange(Number(event.target.value))} {...limits} /></Field>;
}
