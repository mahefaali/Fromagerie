      ...
      <div className="mx-auto max-w-[1440px] space-y-5">
        ...
        {dashboard.loading ? (
          <Card className={OWNER_CARD_CLASS}>
            <CardContent role="status" className="flex items-center gap-3 p-6 text-[#766955]">
...
function DashboardHeader() {
  return (
    <header className="relative overflow-hidden rounded-2xl border border-[#d6cbbb] bg-[linear-gradient(120deg,#254d25_0%,#355f32_62%,#b85c36_145%)] p-4 text-[#fffaf0] shadow-[0_8px_28px_rgba(39,72,35,0.16)] sm:p-5">
      ...
      <div className="relative flex items-start gap-3">
        <span className="mt-1 flex size-9 shrink-0 items-center justify-center rounded-lg border border-white/15 bg-white/10 shadow-inner">
          <ChartNoAxesCombined className="size-4 text-[#f1d2aa]" />
        </span>
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#e8c99f]">Pilotage propriétaire</p>
          <h1 className="mt-1.5 text-xl font-semibold tracking-tight sm:text-2xl">Vue d’ensemble</h1>
...
function DashboardFilters({ dashboard }: { dashboard: DashboardState }) {
  return (
    <Card className={`${OWNER_CARD_CLASS} py-0`}>
      <CardContent className={`grid gap-3 p-4 sm:grid-cols-2 2xl:items-end ${dashboard.period === "custom" ? "2xl:grid-cols-[auto_1.25fr_0.7fr_0.7fr_1.25fr]" : "2xl:grid-cols-[auto_1fr_1fr]"}`}>
        <div className="flex items-center gap-2 text-sm font-medium text-[#6f624f] sm:col-span-2 2xl:col-span-1"><Filter className="size-4" />Filtres</div>
        <label className="text-xs font-semibold text-[#766955]">
          ...
          <Select value={dashboard.period} onValueChange={(value) => dashboard.setPeriod(value as Period)}>
            <SelectTrigger className="mt-1 h-11 rounded-xl border-[#ddd4c6] bg-[#f8f3ea] text-sm"><SelectValue /></SelectTrigger>
