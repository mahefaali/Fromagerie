...
export default function ProfitabilityPage() {
  ...
  return <section className="mx-auto max-w-[1440px] space-y-5 pb-6 text-[#251f19]">
    <header><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#a45d3f]">Pilotage économique</p><h1 className="mt-1.5 text-xl font-semibold tracking-tight sm:text-2xl">Coûts & Rentabilité</h1><p className="mt-1.5 text-sm text-[#756650]">Comprendre ce que coûte la production, ce que rapportent les fromages et les clients.</p></header>

    <div className="space-y-5 pl-2 sm:pl-3 lg:pl-4">
    <Card className="rounded-2xl border-[#d9d0c1] bg-[#f5efe4] py-0"><CardContent className={`grid gap-3 p-4 sm:grid-cols-2 2xl:items-end ${period === "custom" ? "2xl:grid-cols-[auto_1.2fr_0.7fr_0.7fr_1.2fr]" : "2xl:grid-cols-[auto_1fr_1fr]"}`}><div className="flex items-center gap-2 text-sm font-medium sm:col-span-2 2xl:col-span-1 text-[#6f624f]"><Filter className="size-5" />Filtres</div><label className="text-xs font-semibold text-[#766955]">Période<Select value={period} onValueChange={(value) => changePeriod(value as Period)}><SelectTrigger className="mt-1 h-11 rounded-xl border-[#ddd4c6] bg-[#f8f3ea] text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="month">Ce mois</SelectItem><SelectItem value="previous-month">Mois précédent</SelectItem><SelectItem value="three-months">3 derniers mois</SelectItem><SelectItem value="six-months">6 derniers mois</SelectItem><SelectItem value="year">Cette année</SelectItem><SelectItem value="custom">Période personnalisée</SelectItem></SelectContent></Select></label>{period === "custom" && <><label className="text-xs font-semibold text-[#766955]">Du<input aria-label="Date de début" type="date" value={dateDebut} max={dateFin} onChange={(event) => setDateDebut(event.target.value)} className="mt-1 block h-11 w-full rounded-xl border border-[#ddd4c6] bg-[#f8f3ea] px-3 text-sm font-normal" /></label><label className="text-xs font-semibold text-[#766955]">Au<input aria-label="Date de fin" type="date" value={dateFin} min={dateDebut} max={iso(new Date())} onChange={(event) => setDateFin(event.target.value)} className="mt-1 block h-11 w-full rounded-xl border border-[#ddd4c6] bg-[#f8f3ea] px-3 text-sm font-normal" /></label></>}<label className="text-xs font-semibold text-[#766955]">Type de fromage<Select value={fromageId} onValueChange={setFromageId}><SelectTrigger className="mt-1 h-11 rounded-xl border-[#ddd4c6] bg-[#f8f3ea] text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Tous les fromages</SelectItem>{fromages.map((item) => <SelectItem key={item.id} value={String(item.id)}>{item.nom}</SelectItem>)}</SelectContent></Select></label></CardContent></Card>

    {error && <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-800">{error}</div>}

    <Tabs defaultValue="costs" className="space-y-5"><TabsList className="inline-flex h-auto w-max min-w-full justify-start gap-1 bg-[#EAE0D0] p-1 sm:min-w-0"><TabsTrigger value="costs">Coûts de production</TabsTrigger><TabsTrigger value="profitability">Rentabilité</TabsTrigger><TabsTrigger value="indicators">Indicateurs</TabsTrigger></TabsList>
...
function Loading() { return <Card className="rounded-2xl border-[#d9d0c1] bg-[#f5efe4]"><CardContent role="status" className="p-6 text-sm text-[#806f59]">Calcul des données économiques…</CardContent></Card>; }
